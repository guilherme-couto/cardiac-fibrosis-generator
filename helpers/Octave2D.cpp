/*
 * Octave2D.cpp (UPGRADED FOR FLOW NOISE / FIBER ADVECTION)
 *
 * Creates an 'octave noise' pattern. This upgraded version accepts a local 
 * vector field (fibers) to dynamically warp the noise gradients at a microscopic 
 * level. This creates perfect anisotropic flow noise without global space deformation.
*/
...
/* Noise calculation function. NOW SUPPORTS LOCAL FIBER ADVECTION */
double noise2D(double x, double y, int *P, double fx, double fy, double align)
{
    ...
    // =========================================================================
    // LOCAL GRADIENT DEFORMATION (Zero Lever-Arm Flow Noise)
    // =========================================================================
    if (align != 1.0) {
        double sq_a = sqrt(align);
        double inv_a = 1.0 / sq_a;
        double u, w, new_x, new_y;

        // Applies tensorial stretch exactly along the fiber orientation
        // This guarantees perfect feature scaling without macro-scale distortion
        
        // Deform bottom-left
        u = v_dl.x * fx + v_dl.y * fy; w = -v_dl.x * fy + v_dl.y * fx;
        u *= inv_a; w *= sq_a;
        new_x = u * fx - w * fy; new_y = u * fy + w * fx;
        v_dl.x = new_x; v_dl.y = new_y;

        // Deform bottom-right
        u = v_dr.x * fx + v_dr.y * fy; w = -v_dr.x * fy + v_dr.y * fx;
        u *= inv_a; w *= sq_a;
        new_x = u * fx - w * fy; new_y = u * fy + w * fx;
        v_dr.x = new_x; v_dr.y = new_y;

        // Deform top-left
        u = v_ul.x * fx + v_ul.y * fy; w = -v_ul.x * fy + v_ul.y * fx;
        u *= inv_a; w *= sq_a;
        new_x = u * fx - w * fy; new_y = u * fy + w * fx;
        v_ul.x = new_x; v_ul.y = new_y;

        // Deform top-right
        u = v_ur.x * fx + v_ur.y * fy; w = -v_ur.x * fy + v_ur.y * fx;
        u *= inv_a; w *= sq_a;
        new_x = u * fx - w * fy; new_y = u * fy + w * fx;
        v_ur.x = new_x; v_ur.y = new_y;
    }
    // =========================================================================
...
void OctaveNoise2D(double *points, double *noisefield, mwSize n, unsigned int N_freq, double fade_factor, int *Ps, double *offsets, double *fibers, double align)
{
    mwSize i;
    unsigned int j;
    int freq_mult;
    ...
    for (i=0; i<n; i++) {
        ...
        // Extract local fiber direction for this cell
        double fx = 1.0, fy = 0.0;
        if (fibers != NULL) {
            fx = fibers[2*i];       // MATLAB matrix is passed as a 2xN array
            fy = fibers[2*i + 1]; 
        }
        ...
        for (j=0; j<N_freq; j++) {
            ...
            // Notice we pass the local fiber down to the noise cell
            noisefield[i] = noisefield[i] + scale_mult * noise2D(
                points[2*i]*freq_mult - offsets[2*j], 
                points[2*i+1]*freq_mult - offsets[2*j+1], 
                P, fx, fy, align);
        }
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    // NEW ARGUMENTS FOR FLOW NOISE
    double *fibers = NULL;
    double align = 1.0;
    if (nrhs >= 7) {
        fibers = mxGetPr(prhs[5]);
        align = mxGetScalar(prhs[6]);
    }

    plhs[0] = mxCreateDoubleMatrix(1,(mwSize)n_points,mxREAL);
    double *noisefield = mxGetPr(plhs[0]);

    OctaveNoise2D(points, noisefield, (mwSize)n_points, N_freq, fade_factor, Ps, offsets, fibers, align);
}