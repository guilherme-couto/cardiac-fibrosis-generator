...
% WRITECUSTOMMESH Injects procedural arrays into custom mesh files.
%
...
% column to the original .alg file, preserving the anatomical header.
%
% INPUTS:
%   mesh            - Struct containing .type and .original_file path.
...
%   output_filename - String specifying the desired output name (without extension).
    ...
    fprintf('      |-> Exporting fibrosis pattern to custom mesh\n');
    ...
    if strcmpi(mesh.type, 'alg')
        ...
        if status ~= 0
            ...
            fprintf('      |-> [✓] Custom mesh written to: %s\n', out_file);
        end
